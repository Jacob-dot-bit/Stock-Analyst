...
- Every candidate also carries `market_cap`, `debt_ratio`, and `price_history_years` — informational, never fed into `composite_score`. `market_cap` (price × diluted shares) and `debt_ratio` (the Value pillar's own `debt_to_equity` metric, just surfaced) were already computed inside `scoring/service.py::compute_scores` for every instrument; `price_history_years` is a count over already-cached `PriceBar` rows. All three power client-side Pépites filters, same "unknown excluded" convention as every other filter here. See DEVLOG "Decision 3u.72".
- Every candidate also carries `dividend_yield_estimate` — filed dividends-per-share ÷ price (a new `dividend_per_share` fundamentals concept, EDGAR/ESEF), also informational and never scored. Deliberately separate from the scored `dividend_yield` Value-pillar metric, which only works for held/previously-held stocks via lot replay — the two measure different things. See DEVLOG "Decision 3u.73".
- `POST /backfill-dividends` — one-off, resumable catch-up (same batched "click again" convention as `/refresh`) that re-fetches fundamentals for already-evaluated candidates missing the `dividend_per_share` concept specifically — `/refresh`'s own `verified_at`-gated batch would otherwise never revisit them. See DEVLOG "Decision 3u.73".
